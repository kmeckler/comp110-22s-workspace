...
def average(table: list[dict[str, str]], column: str) -> float:
    """Produce the average value of all rows in a specified column."""
    column_list: list[float] = []
    result: float = 0.0

    for row in table:
        item: float = float(row[column])
